...
class FlowEngine:
    ...
    def _wait_for_image(self, step: FlowStep) -> Optional[Tuple[int, int]]:
        ...
        while time.time() - start_time < timeout_sec:
            ...
            elif step.match_scope == SCOPE_SINGLE_SCREEN:
                # 单屏幕（主显示器）
...
class StepEditDialog(tk.Toplevel):
    ...
    def _build_operation_tab(self, parent):
        ...
        # 延时
        f = ttk.LabelFrame(parent, text="延时", padding=8)
        f.pack(fill='x', pady=4)
    ...
    def _on_operation_change(self):
        ...
        # 重新pack到延时之前
        parent = self.click_frame.master
        
        if show_click:
            self.click_frame.pack(fill='x', pady=4, before=parent.winfo_children()[-1])
        if show_btn:
            self.btn_frame.pack(fill='x', pady=4, before=parent.winfo_children()[-1])
        if show_key:
            self.key_frame.pack(fill='x', pady=4, before=parent.winfo_children()[-1])
        if show_combo:
            self.combo_frame.pack(fill='x', pady=4, before=parent.winfo_children()[-1])
        if show_scroll:
            self.scroll_frame.pack(fill='x', pady=4, before=parent.winfo_children()[-1])
        if show_drag:
            self.drag_frame.pack(fill='x', pady=4, before=parent.winfo_children()[-1])
